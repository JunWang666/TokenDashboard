...
| 目录 | 职责 | 状态 |
|------|------|------|
| [cloudflare-hub](cloudflare-hub/README.md) | Workers + D1：usage/quota 存储，ingest/query/credentials API，Access JWT 校验，AES-256-GCM 凭证加密，cron 额度采集（`src/runner/`：Codex、Kimi、MiniMax Token Plan、Z.ai Coding Plan、AnyRouter、OpenAI、DeepSeek、GLM、Copilot、Claude、Cursor） | ✅ 已实现，34 个测试用例 |
...
- [x] M7 runner：MiniMax Token Plan + Z.ai / GLM Coding Plan（5 小时、周额度，兼容新旧接口）
